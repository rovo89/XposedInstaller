...
public final class RepoDb extends SQLiteOpenHelper {
	...
	private RepoDb(Context context) {
		super(context,
				new File(context.getCacheDir(), RepoDbDefinitions.DATABASE_NAME)
						.getPath(),
	...
	public synchronized static void init(Context context,
		...
		if (mInstance != null)
			throw new IllegalStateException(
					RepoDb.class.getSimpleName() + " is already initialized");
	...
	private static String getString(String table, String searchColumn,
			String searchValue, String resultColumn) {
		String[] projection = new String[] { resultColumn };
		String where = searchColumn + " = ?";
		String[] whereArgs = new String[] { searchValue };
		Cursor c = mDb.query(table, projection, where, whereArgs, null, null,
				null, "1");
		if (c.moveToFirst()) {
			...
			c.close();
			throw new RowNotFoundException("Could not find " + table + "."
					+ searchColumn + " with value '" + searchValue + "'");
		}
	}
	...
	public static Map<Long, Repository> getRepositories() {
		...
		String[] projection = new String[] { RepositoriesColumns._ID,
				RepositoriesColumns.URL, RepositoriesColumns.TITLE,
				RepositoriesColumns.PARTIAL_URL, RepositoriesColumns.VERSION, };

		Cursor c = mDb.query(RepositoriesColumns.TABLE_NAME, projection, null,
				null, null, null, RepositoriesColumns._ID);
		while (c.moveToNext()) {
			Repository repo = new Repository();
			long id = c
					.getLong(c.getColumnIndexOrThrow(RepositoriesColumns._ID));
			repo.url = c.getString(
					c.getColumnIndexOrThrow(RepositoriesColumns.URL));
			repo.name = c.getString(
					c.getColumnIndexOrThrow(RepositoriesColumns.TITLE));
			repo.partialUrl = c.getString(
					c.getColumnIndexOrThrow(RepositoriesColumns.PARTIAL_URL));
			repo.version = c.getString(
					c.getColumnIndexOrThrow(RepositoriesColumns.VERSION));
	...
	public static long insertModule(long repoId, Module mod) {
		...
		mDb.beginTransaction();
		try {
			long moduleId = mDb.insertOrThrow(ModulesColumns.TABLE_NAME, null,
					values);
			...
			for (Pair<String, String> moreInfoEntry : mod.moreInfo) {
				insertMoreInfo(moduleId, moreInfoEntry.first,
						moreInfoEntry.second);
			}
	...
	private static long insertModuleVersion(long moduleId,
		...
		values.put(ModuleVersionsColumns.CHANGELOG_IS_HTML,
	...
	public static void deleteAllModules(long repoId) {
		mDb.delete(ModulesColumns.TABLE_NAME, ModulesColumns.REPO_ID + " = ?",
				new String[] { Long.toString(repoId) });
	}
	...
	public static Module getModuleByPackageName(String packageName) {
		...
		String where = ModulesColumns.PREFERRED + " = 1 AND "
				+ ModulesColumns.PKGNAME + " = ?";
		String[] whereArgs = new String[] { packageName };

		Cursor c = mDb.query(ModulesColumns.TABLE_NAME, projection, where,
		...
		long repoId = c
				.getLong(c.getColumnIndexOrThrow(ModulesColumns.REPO_ID));

		Module mod = new Module(mRepoLoader.getRepository(repoId));
		mod.packageName = c
				.getString(c.getColumnIndexOrThrow(ModulesColumns.PKGNAME));
		mod.name = c.getString(c.getColumnIndexOrThrow(ModulesColumns.TITLE));
		mod.summary = c
				.getString(c.getColumnIndexOrThrow(ModulesColumns.SUMMARY));
		mod.description = c
				.getString(c.getColumnIndexOrThrow(ModulesColumns.DESCRIPTION));
		mod.descriptionIsHtml = c.getInt(c
				.getColumnIndexOrThrow(ModulesColumns.DESCRIPTION_IS_HTML)) > 0;
		mod.author = c
				.getString(c.getColumnIndexOrThrow(ModulesColumns.AUTHOR));
		mod.support = c
				.getString(c.getColumnIndexOrThrow(ModulesColumns.SUPPORT));
		mod.created = c
				.getLong(c.getColumnIndexOrThrow(ModulesColumns.CREATED));
		mod.updated = c
				.getLong(c.getColumnIndexOrThrow(ModulesColumns.UPDATED));
		...
		projection = new String[] { ModuleVersionsColumns.NAME,
				...
				ModuleVersionsColumns.MD5SUM, ModuleVersionsColumns.SIZE,
		...
		while (c.moveToNext()) {
			...
			version.name = c.getString(
					c.getColumnIndexOrThrow(ModuleVersionsColumns.NAME));
			version.code = c.getInt(
					c.getColumnIndexOrThrow(ModuleVersionsColumns.CODE));
			version.downloadLink = c.getString(c.getColumnIndexOrThrow(
					ModuleVersionsColumns.DOWNLOAD_LINK));
			version.md5sum = c.getString(
					c.getColumnIndexOrThrow(ModuleVersionsColumns.MD5SUM));
			version.size = c.getLong(
					c.getColumnIndexOrThrow(ModuleVersionsColumns.SIZE));
			version.changelog = c.getString(
					c.getColumnIndexOrThrow(ModuleVersionsColumns.CHANGELOG));
			version.changelogIsHtml = c.getInt(c.getColumnIndexOrThrow(
					ModuleVersionsColumns.CHANGELOG_IS_HTML)) > 0;
			version.relType = ReleaseType.fromOrdinal(c.getInt(
					c.getColumnIndexOrThrow(ModuleVersionsColumns.RELTYPE)));
			version.uploaded = c.getLong(
					c.getColumnIndexOrThrow(ModuleVersionsColumns.UPLOADED));
			mod.versions.add(version);
		}
		c.close();

		// MoreInfo
		projection = new String[] { MoreInfoColumns.LABEL,
				MoreInfoColumns.VALUE, };

		where = MoreInfoColumns.MODULE_ID + " = ?";
		whereArgs = new String[] { Long.toString(moduleId) };

		c = mDb.query(MoreInfoColumns.TABLE_NAME, projection, where, whereArgs,
				null, null, MoreInfoColumns._ID);
		while (c.moveToNext()) {
			String label = c
	...
	public static String getModuleSupport(String packageName) {
		return getString(ModulesColumns.TABLE_NAME, ModulesColumns.PKGNAME,
				packageName, ModulesColumns.SUPPORT);
	}

	public static void updateModuleLatestVersion(String packageName) {
		int maxShownReleaseType = mRepoLoader
				.getMaxShownReleaseType(packageName).ordinal();
		mDb.execSQL(
	...
	public static void updateAllModulesLatestVersion() {
		...
		try {
			...
			Cursor c = mDb.query(true, ModulesColumns.TABLE_NAME, projection,
	...
	public static long insertInstalledModule(InstalledModule installed) {
		...
		return mDb.insertOrThrow(InstalledModulesColumns.TABLE_NAME, null,
				values);
	}
	...
	public static Cursor queryModuleOverview(int sortingOrder,
		...
		String[] projection = new String[] { "m." + ModulesColumns._ID,
				...
				"v." + ModuleVersionsColumns.NAME + " AS "
						+ OverviewColumns.LATEST_VERSION,
				"i." + InstalledModulesColumns.VERSION_NAME + " AS "
						+ OverviewColumns.INSTALLED_VERSION,

				"(CASE WHEN m." + ModulesColumns.PKGNAME + " = '"
						+ ModuleUtil.getInstance().getFrameworkPackageName()
						+ "' THEN 1 ELSE 0 END) AS "
						+ OverviewColumns.IS_FRAMEWORK,

				"(CASE WHEN i." + InstalledModulesColumns.VERSION_NAME
						+ " IS NOT NULL" + " THEN 1 ELSE 0 END) AS "
						+ OverviewColumns.IS_INSTALLED,

				"(CASE WHEN v." + ModuleVersionsColumns.CODE + " > "
						+ InstalledModulesColumns.VERSION_CODE
						+ " THEN 1 ELSE 0 END) AS "
						+ OverviewColumns.HAS_UPDATE, };

		// Conditions
		String where = ModulesColumns.PREFERRED + " = 1";
		String whereArgs[] = null;
		if (!TextUtils.isEmpty(filterText)) {
			where += " AND (m." + ModulesColumns.TITLE + " LIKE ?" + " OR m."
					+ ModulesColumns.SUMMARY + " LIKE ?" + " OR m."
					+ ModulesColumns.DESCRIPTION + " LIKE ?" + " OR m."
					+ ModulesColumns.AUTHOR + " LIKE ?)";

			String filterTextArg = "%" + filterText + "%";
			whereArgs = new String[] { filterTextArg, filterTextArg,
					filterTextArg, filterTextArg };
		}
		...
		// Query
		Cursor c = mDb.query(
				ModulesColumns.TABLE_NAME + " AS m" + " LEFT JOIN "
	...
	private static String getFirstUpdate(boolean framework) {
		String[] projection = new String[] {
				InstalledModulesUpdatesColumns.LATEST_NAME };
		String where = ModulesColumns.PKGNAME + (framework ? " = ?" : " != ?");
		String[] whereArgs = new String[] {
				ModuleUtil.getInstance().getFrameworkPackageName() };
		Cursor c = mDb.query(InstalledModulesUpdatesColumns.VIEW_NAME,
				projection, where, whereArgs, null, null, null, "1");
		String latestVersion = null;
		if (c.moveToFirst())
			latestVersion = c.getString(c.getColumnIndexOrThrow(
					InstalledModulesUpdatesColumns.LATEST_NAME));
		c.close();
	...
	public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL(
				RepoDbDefinitions.SQL_CREATE_INDEX_MODULE_VERSIONS_MODULE_ID);
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		...
		db.execSQL(
				"DROP TABLE IF EXISTS " + InstalledModulesColumns.TABLE_NAME);
		db.execSQL("DROP VIEW IF EXISTS "
				+ InstalledModulesUpdatesColumns.VIEW_NAME);
