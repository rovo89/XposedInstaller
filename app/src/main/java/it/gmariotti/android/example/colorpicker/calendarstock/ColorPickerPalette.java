...
public class ColorPickerPalette extends TableLayout {
	...
	public void init(int size, int columns, OnColorSelectedListener listener) {
		...
		if (size == ColorPickerDialog.SIZE_LARGE) {
			mSwatchLength = res
					.getDimensionPixelSize(R.dimen.color_swatch_large);
			mMarginSize = res
					.getDimensionPixelSize(R.dimen.color_swatch_margins_large);
		} else {
			mSwatchLength = res
					.getDimensionPixelSize(R.dimen.color_swatch_small);
			mMarginSize = res
					.getDimensionPixelSize(R.dimen.color_swatch_margins_small);
		}
		mOnColorSelectedListener = listener;

		mDescription = res.getString(R.string.color_swatch_description);
		mDescriptionSelected = res
	...
	public void drawPalette(int[] colors, int selectedColor) {
		...
		for (int color : colors) {
			...
			setSwatchDescription(rowNumber, tableElements, rowElements,
					color == selectedColor, colorSwatch);
	 ...
	 */
	private void setSwatchDescription(int rowNumber, int index, int rowElements,
			boolean selected, View swatch) {
		...
		String description;
		if (selected) {
			description = String.format(mDescriptionSelected,
					accessibilityIndex);
		} else {
	...
	private ImageView createBlankSpace() {
		...
		TableRow.LayoutParams params = new TableRow.LayoutParams(mSwatchLength,
				mSwatchLength);
